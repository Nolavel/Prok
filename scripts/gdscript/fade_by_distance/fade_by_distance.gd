...
# 🔥 Референс на InputManager
@export_group("References")
@export var input_manager: NodePath

@onready var _material := material as ShaderMaterial
var _prev_player_uv := Vector2(-1, -1)
var _prev_enabled := true
var _camera: Camera3D
var _is_paused := false
var _active_tween: Tween
var _input_manager_node: Node3D

func _ready() -> void:
	if _material:
		_update_static_params()
...
# 🔥 Колбэк для сигнала из InputManager
...
func _animate_to_paused() -> void:
	...
	# 🔥 Замедляем время зерна для sci-fi эффекта
	_active_tween.tween_method(
